...
fn findNumber(prevLine: ?[]u8, currLine: []u8, nextLine: ?[]u8, i: usize) u32 {
    var firstNumber: ?u32 = null;
    var secondNumber: ?u32 = null;
    ...
    return (firstNumber orelse 0) * (secondNumber orelse 0);
}
...
fn findNumbers(prevLine: ?[]u8, currLine: []u8, nextLine: ?[]u8) !u32 {
    ...
    for (currLine, 0..) |char, i| {
        if (char == '*') {
            sum += findNumber(prevLine, currLine, nextLine, i);
        }
    }
